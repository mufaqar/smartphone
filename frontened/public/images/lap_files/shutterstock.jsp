			...
			try {
				...
				if (!usi_app.suppress_page) {
					...
					if (usi_app.is_image_page || usi_app.is_image_in_cart) {
						var threshold = 0;
						if (usi_date.is_after('2024-01-15') && usi_app.check_sub_total_price(threshold) && (usi_app.is_flexable || (usi_app.is_checkout_page && checkout != null && (checkout.textContent.trim().indexOf("FLEX 10") != -1 || checkout.textContent.trim().indexOf("5 Credit Pack") != -1 || checkout.textContent.trim().indexOf("2 Credit Pack") != -1)))) {
							...
							if (usi_app.messaging && usi_app.messaging.header1) {
								usi_commons.log('[ load ] * * * Load Images 10% off - ' + param2 + ' - [language key: ' + language_key + '] * * *');
